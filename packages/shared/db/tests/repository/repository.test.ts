...
import {
  ...
  productToProductRow,
  productToValueRow,
  categoriesAndTheirProducts,
  categoriesAndTheirProductsWithMultiplePricePoints,
} from "./helper.js";
import { PostgreSqlContainer } from "@testcontainers/postgresql";
import { readFile } from "node:fs/promises";
import { Product, ValueAtTime } from "@grocery-tracker/domain-model";
...
describe("CategoryRepository", () => {
  beforeEach(async () => {
    ...
    woolworthsRetailerId = (
      await client.query("SELECT id FROM retailers WHERE name = $1;", [
        "Woolworths",
      ])
    ).rows[0].id;
    colesRetailerId = (
      await client.query("SELECT id FROM retailers WHERE name = $1;", ["Coles"])
    ).rows[0].id;
  });
  ...
  it.each(categoriesAndTheirProducts)(
    "can create a category",
    async (categoryAndProducts) => {
      const categoryRepository = new PostgresCategoryRepository(pool);
  ...
  it.each(categoriesAndTheirProducts)(
    "can create a category and find the result",
    async (categoryAndProducts) => {
      const categoryRepository = new PostgresCategoryRepository(pool);
      const category = categoryAndProducts.category;
      await categoryRepository.createOrUpdate(category);

      const categoriesRepRes = await categoryRepository.findBy(
...
describe("ProductRepository", () => {
  ...
  it("finds a list through a pool with one client", async () => {
    ...
    const categoryRepository = new PostgresCategoryRepository(pool);
    const productRepository: ProductRepository = new PostgresProductRepository(
      pool,
    );
    ...
    const products = Array.from(
      ...
      (_, index) =>
        new Product({
          retailer: categoryAndProducts.category.retailer,
          retailerProductId: `single-client-${index}`,
          category: categoryAndProducts.category,
          currentValue: new ValueAtTime("1 each", index + 1, new Date()),
          name: `single-client-product-${index}`,
          path: `/single-client-${index}`,
          description: `Single client test product ${index}`,
        }),
    );
    ...
    try {
      const singleClientRepository: ProductRepository =
        new PostgresProductRepository(singleClientPool);
      const exactProducts = await singleClientRepository.findBy(
        { key: "retailer", value: categoryAndProducts.category.retailer },
        [0, 19],
      );
      const similarProducts = await singleClientRepository.findSimilarBy(
        { key: "name", value: "single-client-product" },
        [0, 19],
      );

      expect(exactProducts).toHaveLength(20);
      expect(similarProducts).toHaveLength(20);
    } finally {
      await singleClientPool.end();
    }
  });

  it("uses the most recent value when finding products", async () => {
    const categoryAndProducts = categoriesAndTheirProducts[0];
    const categoryRepository = new PostgresCategoryRepository(pool);
    const productRepository: ProductRepository = new PostgresProductRepository(
      pool,
    ...
    const product = new Product({
      retailer: categoryAndProducts.category.retailer,
      retailerProductId: "most-recent-value",
      category: categoryAndProducts.category,
      currentValue: new ValueAtTime("1 each", 1, new Date()),
      name: "Most recent value product",
      path: "/most-recent-value",
      description: "Tests list hydration selects the latest value",
    });
    ...
    const productId = (
      await pool.query(
        "SELECT id FROM products WHERE retailer_product_id = $1",
        [product.retailerProductId],
      )
    ).rows[0].id;
    const mostRecentTime = new Date("2030-01-01T00:00:00.000Z");
    await pool.query(
      "INSERT INTO value_at_times (product_id, time, size, price) VALUES ($1, $2, $3, $4)",
      [productId, mostRecentTime, "1 each", 2],
    );

    const products = await productRepository.findBy({
      key: "retailerProductId",
      value: product.retailerProductId,
    });

    expect(products).toHaveLength(1);
    expect(products[0].currentValue.time).toEqual(mostRecentTime);
  });

  it.each(categoriesAndTheirProducts)(
    "can create a product after its category has been created",
    async (categoryAndProduct) => {
      const categoryRepository = new PostgresCategoryRepository(pool);
      ...
      for (const product of categoryAndProduct.products) {
        ...
        ;`,
          ...
          expect(productRes.rows[0]).toEqual(
        ...
        ;`,
          ...
          const productCategoriesLinkRes = await client.query(
            `SELECT * FROM product_categories WHERE
          product_id = $1 AND
          category_id = $2
        ;`,
          ...
          expect(productCategoriesLinkRes.rows[0]).toEqual(
            expect.objectContaining({
          ...
          const valueRes = await client.query(
            "SELECT * FROM value_at_times WHERE product_id = $1;",
            [productId],
          );
          expect(valueRes.rowCount).toEqual(1);
          expect(valueRes.rows[0]).toEqual(
            expect.objectContaining({
  ...
  it.each(categoriesAndTheirProductsWithMultiplePricePoints)(
    "can create and update the price points for a product",
    async (categoryAndProducts) => {
      ...
      for (const rootProductAndClones of categoryAndProducts.productsAndMultiPricePoints) {
        ...
        try {
          await productRepository.createOrUpdate(
            rootProductAndClones.rootProduct,
          );
        ...
        ;`,
            [
              rootProductAndClones.rootProduct.retailer === "Woolworths"
                ? woolworthsRetailerId
                : colesRetailerId,
              rootProductAndClones.rootProduct.retailerProductId,
            ],
          );
          expect(productRes.rowCount).toEqual(1);
          expect(productRes.rows[0]).toEqual(
        ...
        ;`,
          ...
          const valuesRes = await client.query(
            "SELECT * FROM value_at_times WHERE product_id = $1 ORDER BY time;",
            [productId],
          );
          expect(valuesRes.rowCount).toEqual(11);
          valuesRes.rows.forEach((valueRow, i) => {
            if (i === 0) {
              expect(valueRow).toEqual(
                expect.objectContaining({
              ...
              expect(valueRow).toEqual(
                expect.objectContaining({
                  ...productToValueRow(
                    rootProductAndClones.differentPricePointProducts[i - 1],
                  ),
                }),
              );
            }
  ...
  it.each(categoriesAndTheirProducts)(
    "can create a product with a category that hasn't been created",
    async (categoryAndProduct) => {
      const productRepository: ProductRepository =
        new PostgresProductRepository(pool);
      ...
      try {
        ...
        expect(categoryRes.rows[0]).toEqual(
          expect.objectContaining({
            retailer_id:
              categoryAndProduct.category.retailer === "Woolworths"
                ? woolworthsRetailerId
                : colesRetailerId,
            path: categoryAndProduct.category.path,
            retailer_designated_category_id:
              categoryAndProduct.category.retailerDesignatedCategoryId,
  ...
  it.each(categoriesAndTheirProducts)(
    "can create products and find them",
    async (categoryAndProducts) => {
      const categoryRepository = new PostgresCategoryRepository(pool);
      try {
        await categoryRepository.createOrUpdate(categoryAndProducts.category);
        ...
        for (const product of categoryAndProducts.products) {
          ...
          const findByName = await productRepository.findBy({
            key: "name",
            value: product.name,
          });
          const findByPath = await productRepository.findBy({
            key: "path",
            value: product.path,
          });
          const findByDescription = await productRepository.findBy({
            key: "description",
            value: product.description,
          });
          const findByRetailerProductId = await productRepository.findBy({
            key: "retailerProductId",
            value: product.retailerProductId,
          });
          ...
          const expectToRoughlyMatchTime = (result: Product[]) => {
            expect(
              Math.abs(
                result[0].currentValue.time.getTime() -
          ...
          if (product.imageUrl !== undefined) {
            const findByImageUrl = await productRepository.findBy({
              key: "imageUrl",
          ...
          if (product.brand !== undefined) {
            const findByBrand = await productRepository.findBy({
              key: "brand",
              value: product.brand,
            });
          ...
          if (product.crossProductIdentity !== undefined) {
            const findByCrossProductId = await productRepository.findBy({
              key: "crossProductIdentity",
              value: product.crossProductIdentity,
            });
